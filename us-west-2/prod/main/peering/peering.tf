resource "aws_vpc_peering_connection" "us_west_2_main_vpc_peering_existing" {
  ...
  tags = merge(
    ...
    {
    Name = "us-west-2-prod-main-vpc-peering-existing"
    },
  )
...
resource "aws_route" "vpc_route_existing" {
  ...
  destination_cidr_block = data.terraform_remote_state.us_west_2_main_prod_remote.outputs.vpc_cidr
...
resource "aws_route" "vpc_route_existing_new_vpc" {
  for_each = toset(data.terraform_remote_state.us_west_2_main_prod_remote.outputs.vpc_private_route_table_ids)
  route_table_id            = each.value
