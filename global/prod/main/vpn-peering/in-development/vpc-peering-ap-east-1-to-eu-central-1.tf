resource "aws_vpc_peering_connection" "ap_east_1_to_eu_central_1" {
  ...
  tags = merge(
    ...
    {
    Name = "prod-uberads-ap-east-1-to-eu-central-1"
    },
  )

}

resource "aws_vpc_peering_connection_accepter" "ap_east_1_to_eu_central_1_accepter" {
  provider                  = aws.eu-central-1
  vpc_peering_connection_id = aws_vpc_peering_connection.ap_east_1_to_eu_central_1.id
  auto_accept               = true
  accepter {
    allow_remote_vpc_dns_resolution = true
  }
  tags = merge(
    data.terraform_remote_state.global_prod_uberads_remote.outputs.default_tags,
    {
    Name = "prod-uberads-ap-east-1-to-eu-central-1-accepter"
    },
  )
}

module "ap_east_1_to_eu_central_1_routes" {
