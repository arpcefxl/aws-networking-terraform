resource "aws_vpc_peering_connection" "us_east_1_to_ap_east_1" {
  ...
  peer_vpc_id = data.terraform_remote_state.ap_east_1_uberads_prod_remote.outputs.vpc_id
  ...
  tags = merge(
    ...
    {
    Name = "prod-uberads-us-east-1-to-ap-east-1"
    },
  )
...
resource "aws_vpc_peering_connection_accepter" "us_east_1_to_ap_east_1_accepter" {
  ...
  tags = merge(
    data.terraform_remote_state.global_prod_uberads_remote.outputs.default_tags,
    {
    Name = "prod-uberads-us-east-1-to-ap-east-1-accepter"
    },
  )
}
...
module "us_east_1_to_ap_east_1_routes" {
  ...
  destination_cidr                = data.terraform_remote_state.ap_east_1_uberads_prod_remote.outputs.vpc_cidr
  source_vpc_route_table_ids      = data.terraform_remote_state.us_east_1_uberads_prod_remote.outputs.vpc_private_route_table_ids
  destination_vpc_route_table_ids = data.terraform_remote_state.ap_east_1_uberads_prod_remote.outputs.vpc_private_route_table_ids
